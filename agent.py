...
class Agent:
    ...
    def cool(self, object, target):
        if self.verbose:
            print("Entering cooling({}, {})".format(object, target))
    ...
    def coolObject(self, object:str, filter:FrameParticleFilter)->bool:
        if self.verbose:
            print(
        ...
        topKParticles = filter.getMaxWeightParticles()
        attempts = 0
        while not objCooled and attempts < self.retries:
            currentBestEstimate = topKParticles[0][0]
            navGoal = {
                "x": currentBestEstimate[0],
                "z": currentBestEstimate[1],
            ...
            if obj_id is not None:
                ...
                attempts += 1
                continue
            if self.cool(object, obj_id):
                if self.verbose:
                    print("[AGENT]: Cooled {}!".format(object))
                self.state.action_history.append("Cool_{}".format(object))
                self.updateFilters()
                self.saveDistributions()
                objCooled = True
                return True
                ...
                if self.verbose:
                    print("[AGENT]: Cooling {} Failed".format(object))
                self.updateFilters()
                self.saveDistributions()
                topKParticles = filter.getMaxWeightParticles()
                attempts += 1
    ...
    def execute(self, frame_name: str):
        ...
        elif frame_name.split("_")[0] == "Cool":
            obj = frame_name.split("_")[1]
            if self.heatObject(obj, frameFilter):
                print("Cooled {}".format(obj))
                return True, "Success"
            else:
                return False, self.controller.last_event.metadata["errorMessage"]
