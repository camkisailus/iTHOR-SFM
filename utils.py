...
def loadFramesFromALFRED(goal_description):
    ...
    if "pick_and_place_with_movable_recep" in goal_description:
    ...
    elif "pick_cool" in goal_description:
        # form is pick_cool_then_place_in_recep-{obj}-None-{target}-11
        obj = goal_description.split("-")[1]
        target = goal_description.split("-")[3]
        # if target == "BathtubBasin":
        #     target = "Bathtub"
        ...
        open_fridge_frame = {
            "name": "Open_Fridge",
            "description": "Open a Fridge",
            "frame_elements": [{"name": "Fridge", "is_core": True}],
        }
        frames.append(Frame(open_fridge_frame))
        if slice:
            grasp_obj_frame = {
                "name": "Grasp_{}".format(obj),
                "description": "Grasp a {}".format(obj),
            ...
            frames.append(Frame(grasp_obj_frame))
            cool_obj_frame = {
                "name": "Cool_{}".format(obj),
                "description": "Cool {} in a fridge".format(obj),
                "frame_elements": [
                    {"name": obj, "is_core": True},
                    {"name": "Fridge", "is_core": True},
                ],
                "preconditions": [
                    {"name": "Slice_{}".format(obj.replace("Sliced", ""))},
                    {"name": "Open_Fridge"},
            ...
            frames.append(Frame(cool_obj_frame))
            if target in OPENABLE_RECEPS:
                open_target_frame = {
                "name": "Open_{}".format(target),
                "description": "Open a {}".format(target),
