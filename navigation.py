...
class Navigation:
    ...
    def getRandomValidPose(self):
        valid_pose = self.reachable_poses[
            random.randint(0, len(self.reachable_poses) - 1), :
        ]
    ...
    def planPath(self, robot_pose, goal_pose, tolerance=0.05):
        ...
        dists = np.sqrt(
            np.sum(np.square(np.abs(self.reachable_poses - goal_pose_np)), axis=1)
        )
        if np.min(dists) > tolerance:
            ...
            for i, pose in enumerate(self.reachable_poses):
                node = Node(
                    parent=None, action=None, position=[pose[0], pose[1]], rotation=0.0
                )
            ...
            start_node = Node(
                parent=None,
                action=None,
                position=[robot_pose["x"], robot_pose["z"]],
                ...
                if goal_node.rotation != goal_rotation:
                    ...
                    if diff < 0:
                        ...
                        node = Node(
                            goal_node,
                            "turn_left",
                            (goal_node.x, goal_node.z),
                            goal_node.rotation - 90.0,
                        )
